...
public class EdmxMetaDataWriter extends XmlFormatWriter {
		...
		for (EdmSchema schema : services.getSchemas()) {
			...
			for (EdmComplexType ect : schema.getComplexTypes()) {
				...
				if (null != ect.getIsAbstract()) {
					writer.writeAttribute("Abstract", ect.getIsAbstract()
							.toString());
				}
			...
			for (EdmEntityType eet : schema.getEntityTypes()) {
				...
				if (null != eet.getIsAbstract()) {
					writer.writeAttribute("Abstract", eet.getIsAbstract()
							.toString());
				}

				if (Boolean.TRUE.equals(eet.getHasStream())) {
					writer.writeAttribute(new QName2(m, "HasStream", "m"),
							"true");
				}
				...
				if (eet.isRootType()) {
					...
					writer.writeAttribute("BaseType", eet.getBaseType()
				...
				for (EdmNavigationProperty np : eet
						.getDeclaredNavigationProperties()) {

					writer.startElement(new QName2("NavigationProperty"));
					writer.writeAttribute("Name", np.getName());
					writer.writeAttribute("Relationship", np.getRelationship()
							.getFQNamespaceName());
					writer.writeAttribute("FromRole", np.getFromRole()
							.getRole());
			...
			for (EdmAssociation assoc : schema.getAssociations()) {
				...
				writer.writeAttribute("Type", assoc.getEnd1().getType()
						.getFullyQualifiedTypeName());
				writer.writeAttribute("Multiplicity", assoc.getEnd1()
						.getMultiplicity().getSymbolString());
				writer.endElement("End");

				writer.startElement(new QName2("End"));
				writer.writeAttribute("Role", assoc.getEnd2().getRole());
				writer.writeAttribute("Type", assoc.getEnd2().getType()
						.getFullyQualifiedTypeName());
				writer.writeAttribute("Multiplicity", assoc.getEnd2()
						.getMultiplicity().getSymbolString());
				writer.endElement("End");
			...
			for (EdmEntityContainer container : schema.getEntityContainers()) {
				...
				writer.writeAttribute(new QName2(m, "IsDefaultEntityContainer",
						"m"), Boolean.toString(container.isDefault()));
				...
				for (EdmEntitySet ees : container.getEntitySets()) {
					...
					writer.writeAttribute("EntityType", ees.getType()
							.getFullyQualifiedTypeName());
				...
				for (EdmFunctionImport fi : container.getFunctionImports()) {
					...
					if (null != fi.getEntitySet()) {
						writer.writeAttribute("EntitySet", fi.getEntitySet()
								.getName());
					}
					if (fi.getReturnType() != null) {
						// TODO: how to differentiate inline ReturnType vs
						// embedded ReturnType?
						writer.writeAttribute("ReturnType", fi.getReturnType()
								.getFullyQualifiedTypeName());
					}
					writer.writeAttribute(new QName2(m, "HttpMethod", "m"),
							fi.getHttpMethod());
					...
					for (EdmFunctionParameter param : fi.getParameters()) {
						...
						if (param.getMode() != null)
							writer.writeAttribute("Mode", param.getMode()
									.toString());
				...
				for (EdmAssociationSet eas : container.getAssociationSets()) {
					...
					writer.writeAttribute("Association", eas.getAssociation()
							.getFQNamespaceName());
					writeAnnotationAttributes(eas, writer);
					writeDocumentation(eas, writer);

					writer.startElement(new QName2("End"));
					writer.writeAttribute("Role", eas.getEnd1().getRole()
							.getRole());
					writer.writeAttribute("EntitySet", eas.getEnd1()
							.getEntitySet().getName());
					writer.endElement("End");

					writer.startElement(new QName2("End"));
					writer.writeAttribute("Role", eas.getEnd2().getRole()
							.getRole());
					writer.writeAttribute("EntitySet", eas.getEnd2()
							.getEntitySet().getName());
					writer.endElement("End");
	 ...
	 * of namespaces.
	 */
	private static void writeExtensionNamespaces(EdmDataServices services,
			XMLWriter2 writer) {
	...
	private static void writeProperties(Iterable<EdmProperty> properties,
			XMLWriter2 writer) {
		for (EdmProperty prop : properties) {
			writer.startElement(new QName2("Property"));

			writer.writeAttribute("Name", prop.getName());
			writer.writeAttribute("Type", prop.getType()
					.getFullyQualifiedTypeName());
			writer.writeAttribute("Nullable", prop.isNullable() ? "true"
					: "false");
			if (prop.getMaxLength() != null) {
				writer.writeAttribute("MaxLength",
						Integer.toString(prop.getMaxLength()));
			}
			if (!prop.getCollectionKind().equals(CollectionKind.NONE)) {
				writer.writeAttribute("CollectionKind", prop
						.getCollectionKind().toString());
			}
			if (prop.getDefaultValue() != null) {
				writer.writeAttribute("DefaultValue", prop.getDefaultValue());
			}
			if (prop.getPrecision() != null) {
				writer.writeAttribute("Precision",
						Integer.toString(prop.getPrecision()));
			}
			if (prop.getScale() != null) {
				writer.writeAttribute("Scale",
						Integer.toString(prop.getPrecision()));
			}
	...
	private static void writeAnnotationAttributes(EdmItem item,
			XMLWriter2 writer) {
		if (null != item.getAnnotations()) {
			for (NamespacedAnnotation<?> a : item.getAnnotations()) {
				if (a instanceof EdmAnnotationAttribute) {
					writer.writeAttribute(new QName2(a.getNamespace().getUri(),
							a.getName(), a.getNamespace().getPrefix()), a
							.getValue() == null ? "" : a.getValue().toString());
				}
			}
		}
	}
		...
		if (null != item.getAnnotations()) {
			for (NamespacedAnnotation<?> a : item.getAnnotations()) {
				if (a instanceof EdmAnnotationElement) {
					...
					// this totally breaks ODataConsumer even thought it doesn't
					// rely
					// on annotations. A no-op is a interim approach that allows
					// work
					// to proceed by those using queryable metadata to access
					// annotations.
					// throw new
					// UnsupportedOperationException("Implement element annotations");
				}
			}
		}
	}
