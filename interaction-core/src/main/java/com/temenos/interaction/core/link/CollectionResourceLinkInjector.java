 ...
 * does not wrap an entity that is an instance of {@link RESTResource} it just returns 
 * the entity without injecting any links.
 * @author aphethean
 */
public class CollectionResourceLinkInjector implements HateoasLinkInjector<Object> {
    ...
    public boolean canInject(Object entity) {
    	...
    	return ResourceTypeHelper.isType(ge.getRawType(), ge.getType(), CollectionResource.class);
    }
    ...
    public Object injectLinks(Object entity, LinkProducer<Object> linkProducer, final HateoasVerbosity verbosity) {
    	assert(entity instanceof GenericEntity<?>);
    	GenericEntity<?> ge = (GenericEntity<?>) entity;
    	// must be instance of CollectionResource, see canInject
    	CollectionResource<?> rr = (CollectionResource<?>) ge.getEntity();
    	rr.setLinks(linkProducer.getLinks(entity));
