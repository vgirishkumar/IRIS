...
public class TestResourceStateMachine {
    ...
    public void testCreateLinkHrefUriParameterTokensReplaceUsingEntity() {
        ...
        Transition t = new Transition.Builder().source(mock(ResourceState.class)).target(mockTarget("/test"))
                .uriParameters(uriParameters).build();
        Link result = engine.createLink(t, createTestNote("123"), null);
    ...
    /*
     * Evaluate custom link relation, via the Link header. See (see rfc5988) We
     * return a Link if the header is set and the @{link Transition} can be
     * found.
     */
    ...
    public void testGetLinkForCustomLinkRelation() {
        ...
        ResourceState cookingState = new ResourceState("toaster", "cooking", new ArrayList<Action>(),
                "/machines/toaster/cooking");
    ...
    /*
     * We return a Link if a @{link Transition} for supplied method can be
     * found.
     */
    @SuppressWarnings("unchecked")
    @Test
    public void testGetLinkForTargetState() {
        ...
        Link targetLink = stateMachine.getLinkFromMethod(mock(MultivaluedMap.class), testResponseEntity, cookingState,
                "DELETE");
    ...
    public void testGetLinkForSelfState() {
        CollectionResourceState collectionState = new CollectionResourceState("machines", "MachineView",
                new ArrayList<Action>(), "/machines");
        ...
        Link targetLink = stateMachine.getLinkFromMethod(mock(MultivaluedMap.class), testResponseEntity,
                collectionState, "POST");

        assertNotNull(targetLink);
        // a target link the same as our current state equates to 205 Reset
        // Content
        assertEquals("/baseuri/machines", targetLink.getHref());
        // we use createSelfLink under the covers so link id looks like
        // transition to self
    ...
    /*
     * We return a Link if a @{link Transition} for supplied method can be
     * found. When the target state is a pseudo final state, no link will be
     * returned.
     */
    @SuppressWarnings("unchecked")
    @Test
    public void testGetLinkForFinalPseudoState() {
        ResourceState existsState = new ResourceState("toaster", "exists", new ArrayList<Action>(),
                "/machines/toaster/{id}");
        ...
        Link targetLink = stateMachine.getLinkFromMethod(mock(MultivaluedMap.class), testResponseEntity, existsState,
                "DELETE");
    ...
    public void testStates() {
        ...
        ResourceState unauthorised = new ResourceState(ENTITY_NAME, "INAU", new ArrayList<Action>(),
                "unauthorised/{id}");
    ...
    public void testGetInteractionsByState() {
        ...
        ResourceState noteCreateState = new ResourceState(entityName, "note_create", new ArrayList<Action>(),
                "/note('{id}')/create");
        ...
        ResourceState noteDeleteState = new ResourceState(entityName, "note_delete", new ArrayList<Action>(),
                "/note('{id}')/delete");
        noteState.addTransition(new Transition.Builder().method("DELETE").target(noteDeleteState).build());
        ResourceState noteRestoreState = new ResourceState(entityName, "note_restore", new ArrayList<Action>(),
                "/note('{id}')/restore");
    ...
    public void testGetResourceStatesByPathRegex() {
        ...
        ResourceState notesEntity = new ResourceState(ENTITY_NAME, "notesEntity", new ArrayList<Action>(),
                "/notes({id})");
        ResourceState notesEntityQuoted = new ResourceState(ENTITY_NAME, "notesEntityQuoted", new ArrayList<Action>(),
                "/notes('{id}')");
        ResourceState notesNavProperty = new ResourceState(ENTITY_NAME, "notesEntity", new ArrayList<Action>(),
                "/notes({id})/{navproperty}");
        ...
        assertEquals("Number of states: initial", 1, sm.getResourceStatesForPathRegex("^/notes").size());
        assertEquals("Number of states: notesRegex", 1, sm.getResourceStatesForPathRegex("^/notes(\\(\\))").size());
        assertEquals("Number of states: initial, notesRegex", 2, sm.getResourceStatesForPathRegex("^/notes(|\\(\\))")
                .size());
        assertEquals("Number of states: notesEntity", 1, sm.getResourceStatesForPathRegex("^/notes(|[\\(.\\)])").size());
        assertEquals("Number of states: initial, notesRegex, notesEntityQuoted, and notesEntity", 4, sm
                .getResourceStatesForPathRegex("^/notes(|\\(.*\\))").size());
    ...
    public void testTransitionToStateMachine() {
        ...
        // process behaviour
        ResourceState processes = new ResourceState(PROCESS_ENTITY_NAME, "processes", new ArrayList<Action>(),
                "/processes");
        ...
        ResourceState processInitial = new ResourceState(PROCESS_ENTITY_NAME, "initialProcess",
                new ArrayList<Action>(), "/processes/{id}");
        ResourceState processStarted = new ResourceState(processInitial, "started", new ArrayList<Action>());
        ResourceState nextTask = new ResourceState(PROCESS_ENTITY_NAME, "taskAvailable", new ArrayList<Action>(),
                "/nextTask");
        ...
        // Task states
        ResourceState taskAcquired = new ResourceState(TASK_ENTITY_NAME, "acquired", new ArrayList<Action>(),
                "/acquired");
        ResourceState taskComplete = new ResourceState(TASK_ENTITY_NAME, "complete", new ArrayList<Action>(),
                "/completed");
        ...
        /*
         * acquire task by a PUT to the initial state of the task state machine
         * (acquired)
         */
        ...
        // all target states, including states not for this entity (application
        // states)
    ...
    private InteractionContext createMockInteractionContext(ResourceState state) {
        return new InteractionContext(mock(UriInfo.class), mock(HttpHeaders.class), mock(MultivaluedMap.class),
                mock(MultivaluedMap.class), state, mock(Metadata.class));
    }
    ...
    public void testGetLinksEntityNotFound() {
        ...
        Collection<Link> links = stateMachine.injectLinks(rimHandler, createMockInteractionContext(initial),
                testResponseEntity, headers, metadata);
        assertNotNull(links);
        assertTrue(links.isEmpty());
    }

    /*
     * We use links (hypermedia) for controlling / describing application state.
     * Test we return the link to 'self' correctly for our test resource.
     */
    @Test
    ...
    /*
     * We use links (hypermedia) for controlling / describing application state.
     * Test we return the link to 'self' correctly for our test resource; in
     * this self link we have used a path parameter.
     */
    @SuppressWarnings("unchecked")
    @Test
    public void testGetLinksSelfPathParameters() {
        ...
        HttpHeaders headers = mock(HttpHeaders.class);
        Metadata metadata = mock(Metadata.class);
        Collection<Link> links = stateMachine.injectLinks(rimHandler,
                new InteractionContext(mock(UriInfo.class), mock(HttpHeaders.class), mockPathparameters,
                        mock(MultivaluedMap.class), initial, mock(Metadata.class)), testResponseEntity, headers,
                metadata);
    ...
    /*
     * We use links (hypermedia) for controlling / describing application state.
     * Test we return the links to other resource in our state machine.
     */
    @Test
    public void testGetLinksOtherResources() {
        ...
        CollectionResourceState notesResource = new CollectionResourceState(NOTE_ENTITY, "collection",
                new ArrayList<Action>(), notesResourcePath);
        String PERSON_ENTITY = "PERSON";
        String personResourcePath = "/persons";
        CollectionResourceState personsResource = new CollectionResourceState(PERSON_ENTITY, "collection",
                new ArrayList<Action>(), personResourcePath);
        ...
        HttpHeaders headers = mock(HttpHeaders.class);
        Metadata metadata = mock(Metadata.class);
        Collection<Link> unsortedLinks = stateMachine.injectLinks(rimHandler, createMockInteractionContext(initial),
                new EntityResource<Object>(null), headers, metadata);
    ...
    /*
     * We use links (hypermedia) for controlling / describing application state.
     * Test we return the conditional links to other resource in our state
     * machine.
     */
    @Test
    public void testShowConditionalLinks() {
        ...
        // booking cancelled
        ResourceState cancelled = new ResourceState(initial, "cancelled", new ArrayList<Action>(), "/cancelled",
                "cancelled".split(" "));
        ...
        ResourceState pconfirmed = new ResourceState(paid, "pconfirmed", mockNotFound, "/pconfirmed",
                "confirmed".split(" "));
        ...
        // expressions can be added to the resource with the condition or
        // anywhere in the resource state graph
        ...
        ResourceState cancel = new ResourceState(cancelled, "psuedo_cancel", new ArrayList<Action>(), null,
                "cancel".split(" "));
        ResourceState assignRoom = new ResourceState(room, "psuedo_assignroom", new ArrayList<Action>());
        ResourceState paymentDetails = new ResourceState(paid, "psuedo_setcarddetails", new ArrayList<Action>(), null,
                "pay".split(" "));
        ...
        initial.addTransition(new Transition.Builder().method("PUT").target(paymentDetails)
                .uriParameters(uriLinkageMap).flags(transitionFlags)
                .evaluation(new SimpleLogicalExpressionEvaluator(expressions)).label("Make a payment").build());
        ...
        Metadata metadata = mock(Metadata.class);
        Collection<Link> unsortedLinks = stateMachine.injectLinks(rimHandler, createMockInteractionContext(initial),
                new EntityResource<Object>(new Booking("123")), headers, metadata);

        assertNotNull(unsortedLinks);
        assertFalse(unsortedLinks.isEmpty());
        assertEquals(8, unsortedLinks.size());
        /*
         * expect 4 links 'self' GET room GET cancelled GET paid GET pwaiting
         * GET pconfirmed POST cancellation PUT room & link to PUT pwaiting (as
         * the booking has not been paid 'pconfirmed' and is not waiting
         * 'pwaiting')
         */
        List<Link> links = new ArrayList<Link>(unsortedLinks);
        // sort the links so we have a predictable order for this test
     ...
     * We use links (hypermedia) for controlling / describing application state.
     * Test we return the conditional links to other resource in our state
     * machine; in this scenario the conditional link is only available as a
     * transition from the resource with the condition
     */
    @Test
    public void testShowConditionalLinksTargetOnlyExistsInExpression() {
        ...
        expressions.add(new ResourceGETExpression(pconfirmed, Function.NOT_FOUND));
        expressions.add(new ResourceGETExpression(pwaiting, Function.NOT_FOUND));
        room.addTransition(new Transition.Builder().method("PUT").target(paymentDetails).uriParameters(uriLinkageMap)
                .flags(transitionFlags).evaluation(new SimpleLogicalExpressionEvaluator(expressions))
                .label("Make a payment").build());
        ...
        HttpHeaders headers = mock(HttpHeaders.class);
        Metadata metadata = mock(Metadata.class);
        Collection<Link> unsortedLinks = stateMachine.injectLinks(rimHandler, createMockInteractionContext(room),
                new EntityResource<Object>(new Booking("123")), headers, metadata);
        ...
        /*
         * expect 2 links 'self' (the room) & link to PUT pwaiting (as the
         * booking has not been paid 'pconfirmed' and is not waiting 'pwaiting')
         */
        List<Link> links = new ArrayList<Link>(unsortedLinks);
        // sort the links so we have a predictable order for this test
        Collections.sort(links, new Comparator<Link>() {
    ...
    public void testDontShowConditionalLinks() {
        ...
        ResourceState pconfirmed = new ResourceState(paid, "pconfirmed", mockFound, "/pconfirmed",
                "confirmed".split(" "));
        ...
        initial.addTransition(new Transition.Builder().target(cancelled).build());
        initial.addTransition(new Transition.Builder().flags(Transition.AUTO).target(paid).build());
        // TODO, expressions should also be followed in determining resource
        // state graph
        initial.addTransition(new Transition.Builder().target(pwaiting).build());
        initial.addTransition(new Transition.Builder().target(pconfirmed).build());
        ...
        /*
         * In this test case we are mocking that the 'pwaiting' resource was
         * actually found or OK, rather then NOT_FOUND
         */
        List<Expression> expressions = new ArrayList<Expression>();
        expressions.add(new ResourceGETExpression(pconfirmed, Function.NOT_FOUND));
        expressions.add(new ResourceGETExpression(pwaiting, Function.NOT_FOUND));
        ...
        /*
         * expect 3 links 'self' GET room GET cancelled GET paid GET pwaiting
         * GET pconfirmed POST cancellation PUT room & DO NOT show the payment
         * link, payment should already be confirmed
         */
        List<Link> links = new ArrayList<Link>(unsortedLinks);
        // sort the links so we have a predictable order for this test
        Collections.sort(links, new Comparator<Link>() {
    ...
    /*
     * We use links (hypermedia) for controlling / describing application state.
     * Test we return the links for the collection itself.
     */
    @Test
    public void testGetLinksCollection() {
        ...
        ResourceState noteResource = new ResourceState(NOTE_ENTITY, "item", new ArrayList<Action>(),
                noteItemResourcePath, "item".split(" "));
        /* create the transitions (links) */
        // link to form to create new note
        notesResource.addTransition(new Transition.Builder().method("POST")
                .target(new ResourceState("stack", "new", new ArrayList<Action>(), "/notes/new", "new".split(" ")))
                .build());
        /*
         * define transition to view each item of the note collection no linkage
         * map as target URI element (self) must exist in source entity element
         * (also self)
         */
        Map<String, String> uriLinkageMap = new HashMap<String, String>();
        notesResource.addTransition(new Transition.Builder().flags(Transition.FOR_EACH).method("GET")
                .target(noteResource).uriParameters(uriLinkageMap).build());
        ...
        HttpHeaders headers = mock(HttpHeaders.class);
        Metadata metadata = mock(Metadata.class);
        Collection<Link> unsortedLinks = stateMachine.injectLinks(rimHandler,
                createMockInteractionContext(notesResource), testResponseEntity, headers, metadata);
    ...
    /*
     * We use links (hypermedia) for controlling / describing application state.
     * Test we return the links for items in the collection.
     */
    @Test
    public void testGetLinksCollectionItems() {
        ...
        ResourceState noteResource = new ResourceState(NOTE_ENTITY, "item", new ArrayList<Action>(),
                noteItemResourcePath, "item".split(" "));
        ResourceState noteFinalState = new ResourceState(NOTE_ENTITY, "final", new ArrayList<Action>(),
                noteItemResourcePath, "final".split(" "));
        /* create the transitions (links) */
        /*
         * define transition to view each item of the note collection no linkage
         * map as target URI element (self) must exist in source entity element
        ...
        notesResource.addTransition(new Transition.Builder().flags(Transition.FOR_EACH).method("GET")
                .target(noteResource).uriParameters(uriLinkageMap).build());
        notesResource.addTransition(new Transition.Builder().flags(Transition.FOR_EACH).method("DELETE")
                .target(noteFinalState).uriParameters(uriLinkageMap).build());
        // the items of the collection
        List<EntityResource<Object>> entities = new ArrayList<EntityResource<Object>>();
        entities.add(new EntityResource<Object>(createTestNote("1")));
        entities.add(new EntityResource<Object>(createTestNote("2")));
        ...
        HttpHeaders headers = mock(HttpHeaders.class);
        Metadata metadata = mock(Metadata.class);
        Collection<Link> baseLinks = stateMachine.injectLinks(rimHandler, createMockInteractionContext(notesResource),
                testResponseEntity, headers, metadata);
        ...
        /*
         * expect 6 links - one to self for each note for 'collection notes',
         * one to DELETE each note
         */
        assertFalse(links.isEmpty());
    ...
    public void testGetLinkToCollectionResource() {
        ResourceState airport = new ResourceState("Airport", "airport", new ArrayList<Action>(), "/Airports('{id}')",
                null, new UriSpecification("airport", "/Airports('{id}')"));
        CollectionResourceState flights = new CollectionResourceState("Flight", "Flights", new ArrayList<Action>(),
                "/Airports('{id}')/Flights", null, null);

        Map<String, String> uriLinkageMap = new HashMap<String, String>();
        uriLinkageMap.put("filter", "arrivalAirportCode eq '{code}'");
        uriLinkageMap.put("id", "{code}");
        airport.addTransition(new Transition.Builder().method("GET").target(flights).uriParameters(uriLinkageMap)
                .build());
        ...
        HttpHeaders headers = mock(HttpHeaders.class);
        Metadata metadata = mock(Metadata.class);
        Collection<Link> links = rsm.injectLinks(rimHandler, new InteractionContext(mock(UriInfo.class),
                mock(HttpHeaders.class), pathParameters, mock(MultivaluedMap.class), airport, mock(Metadata.class)),
                new EntityResource<Object>(createAirport("123", "BA")), headers, metadata);
        ...
        assertTrue(containsLink(links, "Airport.airport>GET>Flight.Flights",
                "/baseuri/Airports('123')/Flights?filter=arrivalAirportCode+eq+'123'"));
    }
    ...
    public void testGetMultipleLinksToCollectionResourceWithTokenInQueryParams() {
        ...
        Collection<Link> links = rsm.injectLinks(rimHandler, new InteractionContext(mock(UriInfo.class),
                mock(HttpHeaders.class), pathParameters, mock(MultivaluedMap.class), airport, mock(Metadata.class)),
                new EntityResource<Object>(createAirport("London Luton", "LTN")), headers, metadata);
    ...
    public void testInjectLinksForEachToSameResourceState() {
        CollectionResourceState airports = new CollectionResourceState("Airports", "airports", new ArrayList<Action>(),
                "/Airports()", null, null);
        CollectionResourceState flights = new CollectionResourceState("Flight", "Flights", new ArrayList<Action>(),
                "/Flights()", null, null);
        ...
        airports.addTransition(new Transition.Builder().method("GET").target(flights).uriParameters(uriLinkage)
                .flags(Transition.FOR_EACH).label("arrival").build());
        uriLinkage.put("$filter", "departureAirportCode eq '{code}'");
        airports.addTransition(new Transition.Builder().method("GET").target(flights).uriParameters(uriLinkage)
                .flags(Transition.FOR_EACH).label("departure").build());
        ...
        HttpHeaders headers = mock(HttpHeaders.class);
        Metadata metadata = mock(Metadata.class);
        Collection<Link> collectionLinks = rsm.injectLinks(rimHandler, new InteractionContext(mock(UriInfo.class),
                mock(HttpHeaders.class), pathParameters, mock(MultivaluedMap.class), airports, mock(Metadata.class)),
                collectionResource, headers, metadata);
    ...
    public void testInjectLinksForEachToSameResourceStateUriLinkage() {
        ...
        Map<String, String> uriLinkage = new HashMap<String, String>();
        // just using code because its available on our mock object, real life
        // this is arrivalAirportCode
        uriLinkage.put("id", "{code}");
        airports.addTransition(new Transition.Builder().method("GET").target(airport).uriParameters(uriLinkage)
                .flags(Transition.FOR_EACH).label("origin").build());
        // just using code because its available on our mock object, real life
        // this is departureAirportCode
        uriLinkage.put("id", "{iata}");
        airports.addTransition(new Transition.Builder().method("GET").target(airport).uriParameters(uriLinkage)
                .flags(Transition.FOR_EACH).label("destination").build());
    ...
    public void testGetLinkToCollectionResourceWithReferenceToExistingQueryParam() {
        ...
        CollectionResourceState flights = new CollectionResourceState("Flight", "Flights", new ArrayList<Action>(),
                "/Flights()", null, null);
        CollectionResourceState passengers = new CollectionResourceState("Passenger", "Passengers",
                new ArrayList<Action>(), "/Passengers()", null, null);
        ...
        flights.addTransition(new Transition.Builder().method("GET").target(passengers).uriParameters(uriLinkageMap)
                .build());
        ...
        HttpHeaders headers = mock(HttpHeaders.class);
        Metadata metadata = mock(Metadata.class);
        Collection<Link> airportLinks = rsm.injectLinks(rimHandler, new InteractionContext(mock(UriInfo.class),
                mock(HttpHeaders.class), pathParameters, mock(MultivaluedMap.class), airport, mock(Metadata.class)),
                new EntityResource<Object>(createAirport("London Luton", "LTN")), headers, metadata);
        ...
        for (Link airportLink : airportLinks) {
            ...
            Collection<Link> flightsLinks = rsm.injectLinks(rimHandler, new InteractionContext(mock(UriInfo.class),
                    mock(HttpHeaders.class), pathParameters, queryParameters, flights, mock(Metadata.class)),
                    new EntityResource<Object>(null), headers, metadata);

            if (airportLink.getId().equals("Airport.airport>GET(arrivalAirportCode eq '{code}')>Flight.Flights")) {
                assertTrue(containsLink(flightsLinks, "Flight.Flights>GET({myfilter})>Passenger.Passengers",
                        "/baseuri/Passengers()?myfilter=arrivalAirportCode+eq+'London+Luton'"));
            } else if (airportLink.getId().equals(
                    "Airport.airport>GET(departureAirportCode eq '{code}')>Flight.Flights")) {
                assertTrue(containsLink(flightsLinks, "Flight.Flights>GET({myfilter})>Passenger.Passengers",
                        "/baseuri/Passengers()?myfilter=departureAirportCode+eq+'London+Luton'"));
            }
        }
    }
    ...
    public void testEmbedMultipleResourcesEntityResource() {
        ...
        ResourceState childResource1 = new ResourceState("PROFILE", "childResource1", mockActions, "/root/profile",
                "profile".split(" "));
        ResourceState childResource2 = new ResourceState("PREFERENCE", "childResource2", mockActions,
                "/root/preferences", "preferences".split(" "));
        ResourceState childResource3 = new ResourceState("POSTPREF", "postpref", mockActions, "/root/postpref",
                "postpref".split(" "));
        /* create the transitions (links) */
        parentResource.addTransition(new Transition.Builder().flags(Transition.EMBEDDED).method("GET")
                .target(childResource1).build());
        parentResource.addTransition(new Transition.Builder().flags(Transition.EMBEDDED).method("GET")
                .target(childResource2).build());
        parentResource.addTransition(new Transition.Builder().flags(Transition.EMBEDDED).method("POST")
                .target(childResource3).build());
        ...
        Map<Transition, RESTResource> embeddedResources = stateMachine.embedResources(rimHandler,
                mock(HttpHeaders.class), mockCtx, testResponseEntity);
    ...
    public void testGetTransitionProperties() {
        ...
        existsState.addTransition(new Transition.Builder().method("GET").target(cookingState)
        ...
        // Evaluate test
        Map<String, Object> transProps = stateMachine.getTransitionProperties(existsState.getTransition(cookingState),
                entity, pathParameters, null);
        assertEquals("abc", transProps.get("pathParam")); // Check path
                                                          // parameter
        assertEquals("def", transProps.get("linkParam")); // Check link
                                                          // parameter
    ...
    public void testGetTransitionPropertiesWithSameEntityProperty() {
        ...
        customerState.addTransition(new Transition.Builder().method("GET").target(customerState)
                .uriParameters(uriLinkageMap).build());
        ...
        Map<String, Object> transProps = stateMachine.getTransitionProperties(
                customerState.getTransition(customerState), entity, new MultivaluedMapImpl<String>(), null);
    ...
    public void testGetPathParametersForTargetState() {
        ...
        Map<String, Object> transProps = stateMachine.getTransitionProperties(existsState.getTransition(cookingState),
                entity, pathParameters, null);
        MultivaluedMap<String, String> pathParams = HypermediaTemplateHelper.getPathParametersForTargetState(
                existsState.getTransition(cookingState), transProps);
