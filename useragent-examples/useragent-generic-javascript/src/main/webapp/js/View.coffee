define 'cs!View', ['exports', 'jquery', 'underscore', 'cs!Link', 'cs!CRUDLinkFactory'], (exports, $, _, Link, CRUDLinkFactory) ->
  ...
  class View
    ...
    createLink: (resource, model) ->
      result = null
      if model instanceof Link.Link
      	result = model
      else
        factory = new CRUDLinkFactory.CRUDLinkFactory()
